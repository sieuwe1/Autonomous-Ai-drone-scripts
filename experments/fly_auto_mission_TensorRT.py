...
model_dir ="/home/drone/Desktop/Autonomous-Ai-drone-scripts/Tools/modelData.onnx"
...
def predict(img, data):
    ...
    feed_dict = {'img_in': sample_to_predict[0].astype(np.float32), 'path_distance_in': sample_to_predict[1].astype(np.float32)}
    preds = infer(**feed_dict)

    #preds = model({'image': sample_to_predict[0], 'path_distance_in': sample_to_predict[1]})
    #preds = sess.run(None, feed_dict={'image': sample_to_predict[0], 'path_distance_in': sample_to_predict[1]})
...
def init():
    ...
    pb_fname = "/home/drone/Desktop/Autonomous-Ai-drone-scripts/Tools/tensorRT_model"
    model = tf.saved_model.load(pb_fname)
