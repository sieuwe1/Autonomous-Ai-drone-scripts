...
target = (51.4509137,5.4549553) #first run get_gps.py to get target location
...
def write_train_data(left_img, right_img, bgrd_img, vel_x, vel_y, vel_z, alt, target_distance, path_distance, heading_delta, roll, pitch, throttle, yaw, framecount, mode):
    ...
    # print(yaw)
    # print(framecount) `   
    json_data = {"user/roll": roll, "user/pitch": pitch, "user/throttle": throttle, "user/yaw": yaw, "imu/vel_x": vel_x, "imu/vel_y": vel_y, "imu/vel_z": vel_z, "gps/target_distance": target_distance, "gps/path_distance": path_distance, "gps/path_distance": heading_delta, "gps/altitude": alt, "cam/image_name": cam_name, "Ai/mode": mode, "framecount/count": framecount , "user/mode": "user"}
...
def flight_record():
    ...
    while drone.read_channel(record_button_channel) > 1500:
        ...
        write_train_data(left_img, right_img, bgrd_img, vel_x, vel_y, vel_z, alt, target_distance, path_distance, heading_delta, roll, pitch, throttle, yaw, frame_count, 1)
