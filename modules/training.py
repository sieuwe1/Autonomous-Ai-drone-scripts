...
class data_generator(keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        if self.train == True:
            # load train data
            hf = h5py.File(c.train_dir + "/" + train_file_names[index], 'r')
            img_x_train = np.array(hf.get('img_x_train'))
            data_x_train = np.array(hf.get('data_x_train'))
            y_roll_train = np.array(hf.get('y_roll_train'))
            y_pitch_train = np.array(hf.get('y_pitch_train'))
            ...
            hf = h5py.File(c.val_dir + "/" + val_file_names[index], 'r')
            img_x_val = np.array(hf.get('img_x_val'))
            data_x_val = np.array(hf.get('data_x_val'))
            y_roll_val = np.array(hf.get('y_roll_val'))
            y_pitch_val = np.array(hf.get('y_pitch_val'))
...
# create generator
train_generator = data_generator(c.training_size, c.training_size, c.batch_size, True)
val_generator = data_generator(c.validation_size, c.validation_size, c.batch_size, False)
...
history = model.fit_generator(generator=train_generator, steps_per_epoch=int(c.training_size // c.batch_size) - 1,
                              epochs=150,
                              verbose=1, validation_data=val_generator,
                              validation_steps=int(c.validation_size // c.batch_size) - 1,
                              # Why -1? can someone research this?
...
model.save_weights("weights_" + c.model_name)
...
plt.show()
plt.savefig(c.model_name + '_accuracy.png')
...
plt.legend(['train', 'test'], loc='upper left')
plt.show()
plt.savefig(c.model_name + '_loss.png')
