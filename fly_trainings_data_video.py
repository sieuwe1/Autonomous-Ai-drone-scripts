...
def setup_writer():
    ...
    cam_left_dir = data_dir + "/left.avi"
    cam_right_dir = data_dir + "/right.avi"
    cam_depth_dir = data_dir + "/depth.avi"

    left_writer = cv2.VideoWriter(cam_left_dir,cv2.VideoWriter_fourcc('M','J','P','G'),30,(1280,720))
    right_writer = cv2.VideoWriter(cam_right_dir,cv2.VideoWriter_fourcc('M','J','P','G'),30,(1280,720))
    depth_writer = cv2.VideoWriter(cam_depth_dir,cv2.VideoWriter_fourcc('M','J','P','G'),30,(1280,720))
...
def write_train_data(left_img, right_img, depth_img, target_distance, path_distance, roll, pitch, throttle, yaw, framecount):
    left_cam = "yolo"  #write_image(left_img,cam_left_dir,framecount)
    right_cam = "yolo" #write_image(right_img,cam_right_dir,framecount)
    depth_cam = "yolo" #write_image(depth_img,cam_depth_dir,framecount) 
...
def init():
    ...
    drone.connect_drone('/dev/ttyACM1')
